Intended audience: People who come from Java background and just starting to explore Scala.

Hi. There are plenty of articles in the Internet about Monads but nothing is simple enough to understand for a specific set of such audience I mentioned above (like the past me several years back and like my newly joined colleagues).
...
> Monads are data structures with "built-in imperative operations" to let you express your code in a more readable declarative way than hard to process (in your mind) imperative way to do the same thing.

Don't worry if it didn't make sense. Let's look at examples and then come back to this statement.

Here is a Java method that is a sum of squares of even elements in a list (which could be null as well)..
...
Note that if you didn't need to use Option, the code might actually look better like this. (I hate to use flatten cos it sounds more technical than being expressive)
...
* Both containers have the imperative operations like filtering, transforming (map) and aggregating (sum) "built-in" or "baked-in" INSIDE the data type itself so you don't have to do that. Now your code looks just like a chain of operations that you want to happen.
